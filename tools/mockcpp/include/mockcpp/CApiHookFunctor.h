...
#else
typedef void (*report_func_type)(const char *file, unsigned int line, const char *msg);
extern report_func_type reportFailureThrowsNothing;
#define CAPIHOOK_FUNCTOR_DEF(n) \
template <typename R DECL_TEMPLATE_ARGS(n)> \
struct CApiHookFunctor<R(DECL_ARGS(n))> \
{ \
    ...
    { \
        ...
        { \
            reportFailureThrowsNothing( reportedException.getSrcFile().c_str() \
                                 , reportedException.getSrcLine() \
                                 , reportedException.getMessage().c_str()); \
        } \
        catch (std::exception &otherException) \
        { \
            reportFailureThrowsNothing( __FILE__ \
                                 , __LINE__ \
                                 , otherException.what()); \
        } \
        catch (...) \
